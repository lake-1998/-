# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

**无忧存项目知识库** — O2O 寄存平台「小铁寄存柜 + 无忧存」合并项目的交接知识库。这是一个**纯文档仓库**，无可运行代码，无构建/测试流程。

**核心业务**：连接用户与全国 267 城市 3353+ 寄存网点（人工寄存 + 智能寄存柜）的 O2O 平台。

---

## 开始新会话时必读的文档

按顺序读取，快速建立上下文：

| 优先级 | 文档路径 | 核心内容 |
|--------|---------|---------|
| 🔴 必读 | `ClaudeCodeSpace/方案/用户端版本合并UI优化/用户端-版本合并UI优化综合需求文档-20260310.md` (v1.1) | **最新综合需求文档**：对照实装 UI 逐页列出待改动项，可直接交付研发 |
| 🔴 必读 | `ClaudeCodeSpace/方案/用户端版本合并UI优化/用户端-版本合并UI优化方案-20260306.md` (v2.1) | UI 优化方案主文档，含所有已确认的产品决策 |
| 🔴 必读 | `ClaudeCodeSpace/方案/用户端版本合并UI优化/用户端-版本合并UI交互规范PRD-20260309.md` (v1.0) | 页面状态定义、跳转逻辑、边界场景，可直接交给开发 |
| 🟡 重要 | `ClaudeCodeSpace/工作日志/工作日志-20260311.md` | 最新工作会话记录，含未决事项和下次继续点 |
| 🟡 重要 | `002-产品与技术现状 (Product & Tech Stack)/02-产品架构图/无忧存业务全景架构列表.md` | 用户端/商家端/管理后台完整功能清单 |
| 🟢 参考 | `002-产品与技术现状 (Product & Tech Stack)/04-PRD文档/【小铁无忧存】平台合并项目计划-202510.01.md` | 合并项目目标、里程碑、数据依据 |
| 🟢 参考 | `004-协作流程与制度 (Process & Culture)/无忧存开发及发布流程.md` | Git 分支策略、Jenkins CI/CD、三端发布流程 |

---

## 平台合并核心认知（必须正确理解）

**两条完全独立的用户路径**：

| 路径 | 入口 | 流程 | 经过首页？ |
|------|------|------|----------|
| 扫码流 | 扫机柜/网点二维码（URL含 `box_id` 参数）| 扫码 → **直接跳下单页（P04B）** | ❌ 不经过首页 |
| 搜索流 | 运营入口/搜索/分享/直接进入 | 首页 → 搜索 → 列表 → 详情 → 下单 | ✅ 经过首页 |

> ⚠️ **常见错误**：v1.0 方案曾误设计"扫码进入首页并展示特殊布局"（场景A/B/C）——此逻辑已废弃。首页只服务搜索流用户。

**信息解锁策略（防爬/防飞单）**：
- 精确地址、商家电话、实景大图 → **下单后解锁**（非登录后解锁）
- 地图精确点位 → **始终可见**（锁定的是导航能力，不是位置显示）
- 列表页/首页缩略图 → **不模糊**；详情页实景大图 → 模糊遮罩

---

## ClaudeCodeSpace 工作目录

Claude 产出文档的工作区。**新文档按以下结构存放**（2026-03-09 起执行）：

```
ClaudeCodeSpace/
├── 方案/
│   ├── 用户端版本合并UI优化/   # UI 优化相关方案文档 + 设计稿
│   ├── 用户旅程地图/           # 用户旅程地图各版本
│   ├── images/                 # 方案配图
│   └── 防飞单策略整理-20260306.md
├── 工作日志/      # 会话记录（工作日志-YYYYMMDD.md）
├── .claude/       # BMad 斜杠命令（勿移动）
└── _bmad/         # BMad 工具核心（勿移动）
```

> **存档规则**：UI 优化方案文档 → `方案/用户端版本合并UI优化/`；旅程地图 → `方案/用户旅程地图/`；会话记录 → `工作日志/`；设计稿（.pen）→ `方案/用户端版本合并UI优化/`（与配套文档同目录）。

### 用户端UI优化目录（`ClaudeCodeSpace/方案/用户端版本合并UI优化/`）

| 文件 | 版本 | 说明 |
|------|------|------|
| `用户端-版本合并UI优化综合需求文档-20260310.md` | v1.1 | **最新综合需求文档**：对照实装 UI 的逐页改动清单，含验收标准，可直接交研发 |
| `用户端-版本合并UI优化方案-20260306.md` | v2.1 | UI 优化主方案，含首页/列表/详情/下单/订单各页面方案 |
| `用户端-版本合并UI交互规范PRD-20260309.md` | v1.0 | PRD 级交互规范，含状态机/跳转逻辑/边界场景 |
| `用户端-版本合并UI优化改动说明-v1.0-20260309.md` | v1.0 | 改动详情，含接口需求、验收标准 |
| `用户端-版本合并UI-20260309-v1.0.pen` | v1.0 | 设计稿（版本合并 v1.0 基线） |
| `用户端-版本合并UI-20260309-v2.0.pen` | v2.0 | 设计稿（lake 产品反馈后修订版） |
| `用户端-版本合并UI优化原型-20260310-v2.1.pen` | v2.1 | 设计稿（截图核实后最终版，含 P01-P04A） |

### 用户旅程地图目录（`ClaudeCodeSpace/方案/用户旅程地图/`）

| 文件 | 版本 | 说明 |
|------|------|------|
| `无忧存用户旅程地图梳理-v1.0-20260306.md` | v1.0 | 六阶段用户旅程 + P0/P1/P2 痛点矩阵（原始版） |
| `无忧存用户旅程地图-v2.1-20260310.md` | v2.1 | 旅程地图主文档（对齐 v2.1 确认结论） |
| `无忧存用户旅程地图-v2.1-20260310.html` | v2.1 | 旅程地图可视化（HTML 版） |
| `无忧存小程序用户操作流程与流失路径图-v2.0-20260310.html` | v2.0 | 用户操作流程与流失路径（HTML 可视化） |
| `无忧存用户端操作流程与流失路径图-v2.1-20260311.md` | v2.1 | 用户操作流程文字版（含流失率估算） |

### 防飞单策略文档（`ClaudeCodeSpace/方案/`）

| 文件 | 版本 | 说明 |
|------|------|------|
| `防飞单策略整理-20260306.md` | v1.0 | 10项防飞单策略，含 ROI 测算 |

### 工作日志（`ClaudeCodeSpace/工作日志/`）

每次会话结束后创建/更新日志文件（格式：`工作日志-YYYYMMDD.md`），记录：
- 今日完成内容
- 产品侧确认的关键结论
- 未决议题
- 下次继续用的上下文速查

---

## BMad 工具（已安装）

安装路径：`ClaudeCodeSpace/_bmad/`，配置路径：`ClaudeCodeSpace/.claude/commands/`

可用斜杠命令：

| 命令 | 用途 |
|------|------|
| `/bmad-party-mode` | 召集多专家虚拟讨论（Annie/Dana/Marcus/Wei 四个角色）|
| `/bmad-brainstorming` | 结构化头脑风暴 |
| `/bmad-review-adversarial-general` | 批判性审查（对方案做对抗性评估）|
| `/bmad-review-edge-case-hunter` | 边界案例猎手审查 |
| `/bmad-editorial-review-prose` | 文字质量审查 |
| `/bmad-editorial-review-structure` | 结构审查 |

Party Mode 专家团队（已建立，可直接恢复）：🎨 Annie Chen（UX）/ 📊 Dana Liu（数据PM）/ 👤 Marcus（用研）/ 💻 Wei Zhang（前端架构）

---

## 文档写作规范

**分析输出时，区分三类信息**：
1. **事实**（附来源文件路径）
2. **推断**（明确标注为推断）
3. **待确认项**（单独列出，不混入结论）

**数字相互矛盾时**：两个数字都报出来，并说明可能原因（时间窗口差异或指标口径差异）。

**不要凭空发明 KPI 或时间节点**，所有数据必须来自仓库文档。

---

## 技术平台（参考）

**用户端**：微信小程序 / 支付宝小程序 / 抖音小程序 / H5
**商家端**：微信小程序/公众号
**管理后台**：PC Web（测试：`https://noproblem-web.wegui.cn`）

**Git 分支策略**（代码仓库，非本知识库）：

| 分支 | 用途 |
|------|------|
| `master` | 主开发分支 |
| `test-master` | 自动部署测试环境（Jenkins CI/CD）|
| `pre-master` | 预发布分支 |
| `feat-{tapd_id}-{initials}` | 功能分支 |
| `fix-{tapd_id}-{initials}` | 修复分支 |

---

## 关键数据基线

来源：`003-用户与运营数据 (User & Operations)/历史埋点数据分析/无忧存运营情况报告.md`

- 首页搜索使用率：55%+（名称搜索 35.63% + 查找按钮 19.76%）
- "附近"模块点击率：4.66%（严重低于预期）
- 微信搜索流量占比：60%
- 估算飞单率：20-30%

---

## 版本控制原则（本知识库）

- 文档版本用文件名日期管理，不覆盖旧版：`文档名-YYYYMMDD.md`
- 重大修订在文档头部维护 `更新日志` 表格（含版本号、日期、变更内容）
- 新内容优先写入 `ClaudeCodeSpace/`，不直接修改 `002-004` 目录的原始文档
